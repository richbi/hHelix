...
makeHelix <- function(t = NULL,
                      n.loops = c(2, 12, 30),
                      n.points = 10, # n.points: number of points to use to construct the smallest loop (highest order). More points: smoother and slower
                      radii = c(2, 0.5, 0.1),
                      h = NULL,
                      plot = FALSE,
                      tangent = TRUE,
  ...
  if (is.null(t))
    t <- seq(0, 2 * pi * n.loops[1], length.out = n.points * prod(n.loops))
  ...
  if (length(radii) > 1) {
    #---------------------------------------
    #---------------------------------------
    #---------------SHORTER-FASTER SECOND ORDER (TO SPEED THINGS UP LATER) --->
    #---------------------------------------
    #---------------------------------------
